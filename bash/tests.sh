...
test -x /etc/resolv.conf && echo "/etc/resolv.conf is executeable" || echo "/etc/resolv.conf is not executeable"
...
[ -w /tmp ] && echo "/tmp is writeable" || echo "/tmp is not writeable"
[ -r /tmp -a -w /tmp ] && echo "/tmp is accessible" || echo "/tmp is not accessible"
...
if [ /etc/hosts -nt /etc/resolv.conf ]; then
	...
	echo "/etc/resolv.conf are of the same age"
fi 
...
echo "Please give me the first number";read firstNumber
echo "Please give me the second number";read secondNumber
...
isMultiple=$(echo "scale=2; $secondNumber/$firstNumber" | bc)
a=${isMultiple#*.}
#echo $a

if [[ $a -eq 00 ]]; then
	echo "Second number is a multiple of the first"
else
	echo "Second number is not a multiple of the first"
fi
...
[[ ! -z USER ]] && echo "The contents of USER: " $USER
[[ -z USER-unset ]] && echo "USER is empty"
...
echo "Please give me a value for a"; read a
echo "Please give me a value for b"; read b
