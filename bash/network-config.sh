...
myHost=$(hostname)
myAddress=$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')
myHostname=$(getent hosts $(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/\/.*/,"");print $2}')| awk '{print $2}')
externalIP=$(curl -s icanhazip.com)
exterName=$(getent hosts $(curl -s icanhazip.com) | awk '{print $2}')
rtrAddress=$(ip route|grep default|awk '{print $3}')
rtrName=$(fgrep $rtrAddress /etc/hosts|awk '{print $2}')

cat <<EOF
Hostname        :$myHost
LAN Address     :$myAddress
LAN Hostname    :$myHostname
External IP     :$externalIP
External Name   :$exterName
